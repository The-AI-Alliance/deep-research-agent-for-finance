...
Finally, edit the medical configuration YAML files, `mcp_agent.config*.yaml`, in [`dra-apps/medical/config`](https://github.com/The-AI-Alliance/deep-research-agent-for-applications/blob/main/dra-apps/medical/config). Find the configuration for `medical-mcp` in each one and change the path in the `args:` line to match your path. Here is the default setting:
...
The `app-run-medical` target does some setup and then runs the command `cd dra-apps && uv run -m medical.main ...` where `...` is a lot of arguments. 
