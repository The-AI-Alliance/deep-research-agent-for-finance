...
The `app-run-medical` target does some setup and then runs the command `cd apps/src && uv run -m dra.apps.medical.main ...` where `...` is a lot of arguments. 
...
cd apps/src && uv run -m dra.apps.medical.main --help
