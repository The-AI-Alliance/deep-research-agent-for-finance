...
Finally, edit the medical configuration YAML files, `mcp_agent.config*.yaml`, in [`src/dra/apps/medical/config`](https://github.com/The-AI-Alliance/deep-research-agent-for-applications/blob/main/src/dra/apps/medical/config). Find the configuration for `medical-mcp` in each one and change the path in the `args:` line to match your path. Here is the default setting:
...
Application help provided by src/dra/apps/medical/main.py:
